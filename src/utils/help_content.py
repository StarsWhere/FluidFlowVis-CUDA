...
def get_analysis_help_html() -> str:
    ...
    <body>
            ...
            <ul>
                ...
                <li><b>用途:</b>
                    <ul>
                        ...
                        <li><b>现象隔离:</b> 只分析高压区 (<code>p > 101325</code>) 或高速区 (<code>sqrt(u*u+v*v) > 100</code>)。</li>
                        <li><b>多相流:</b> 如果您有一个 `phase_id` 变量，可以只分析某一相，例如 <code>phase_id = 1</code>。</li>
                    </ul>
                </li>
            </ul>
            <div class="note">
                <p><b>重要提示:</b>
                    <ul>
                        <li>应用或更改过滤器后，所有缓存都将失效，数据会从数据库重新加载。</li>
        ...
        <div class="section-box">
            ...
            <ul>
                <li>
                    ...
                    默认模式，显示单个时间步（帧）的数据。这是观察非定常流动、涡旋脱落等瞬态现象的标准模式。
                </li>
                <li>
                    <b>时间平均场 (Time-Averaged Field):</b>
                    计算并显示一个指定时间范围（从起始帧到结束帧）内所有数据点的<b>算术平均值</b>。
                    这对于观察稳态特征、进行雷诺平均(RANS)分析、消除高频湍流脉动非常有用。
                </li>
            </ul>
        </div>
        ...
        <div class="section-box">
            ...
            <p>此功能允许您指定数据中的<b>哪一列</b>作为时间演化的依据，打破了必须按文件名顺序播放的限制。</p>
            <ul>
                <li><b>默认值:</b> <code>frame_index</code> (文件导入的顺序)。</li>
                <li>
                    <b>选择:</b> 您可以从下拉菜单中选择<b>任何数值型变量</b> (例如您数据中的 `simulation_time`, `cycle`, `crank_angle`) 作为新的时间轴。
                </li>
                <li><b>影响范围:</b>
                    <ul>
                        <li><b>播放控制:</b> 播放滑块和播放器将按照所选变量的<b>数值从小到大</b>的顺序进行播放。</li>
                        <li><b>时间序列分析:</b> 在时间序列图中，<b>X轴</b>将变成您所选的这个变量。</li>
                        <li><b>图表标题:</b> 标题中的 <code>{{time}}</code> 占位符将显示所选变量的当前值。</li>
                    </ul>
                </li>
            </ul>
        </div>
        ...
        <div class="section-box">
            ...
            <p>当您在图表上移动鼠标时，“数据探针”窗口会实时显示两种信息，为您提供双重洞察：</p>
            <ul>
                <li>
                    <b>最近原始数据点:</b>
                    显示离您鼠标指针<b>最近的那个原始数据点</b>的所有变量值。
                    这反映了您数据文件中的“真相”。
                </li>
                <li>
                    <b>鼠标位置插值数据:</b>
        ...
        <div class="section-box">
            ...
            <p>此功能允许您在2D图上画一条直线，并立即查看各个物理量如何沿着这条线变化。</p>
            <h4>操作步骤:</h4>
            <ol>
                <li>点击“<b>绘制剖面图</b>”按钮，或通过“按坐标绘制剖面...”菜单项输入精确坐标。</li>
                <li>如果点击按钮，鼠标将变为十字形。</li>
                <li>在图表上<b>第一次点击</b>，定义剖面线的<b>起点</b>。</li>
                <li>移动鼠标，您会看到一条红色的预览虚线。<b>第二次点击</b>，定义剖面线的<b>终点</b>。</li>
                <li>
                    一个新窗口会弹出，显示主热力图变量沿着这条线的数值分布图。
                    您可以在新窗口的下拉菜单中切换查看其他任何已计算场（如等高线场、矢量分量场）的剖面。
                </li>
            </ol>
        </div>
        ...
        <div class="section-box">
            ...
            <p>此功能用于“钻取”数据，查看图上某一个固定点上的物理量随时间的变化情况，并进行频域分析。</p>
            <h4>操作步骤:</h4>
            <ol>
                <li>点击“<b>拾取时间序列点</b>”按钮，或通过“按坐标拾取...”菜单项输入精确坐标。</li>
                <li>在图表上单击您感兴趣的位置。</li>
                <li>
                    一个新窗口会弹出，显示该点附近（在一个小容差范围内）的平均物理量随时间的变化。
                    X轴是当前选定的“时间轴变量”。
                </li>
                <li>您可以在新窗口的下拉菜单中选择不同的变量进行查看。</li>
                <li>
                    点击“<b>计算FFT</b>”按钮，可以对当前显示的时间序列进行<b>快速傅里叶变换</b>，
                    分析其频域特性，帮助您找到流动中的主导频率。
                </li>
            </ol>
        </div>
    </body>

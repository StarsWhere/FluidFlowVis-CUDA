...
class InterpolationWorker(QRunnable):
    def __init__(self, data, res, x_ax, y_ax, x_formula, y_formula, heat_cfg, contour_cfg, vector_cfg, use_gpu, validator):
        super().__init__()
    ...
    def _perform_interpolation(self):
        ...
        # --- 热力图、等高线、矢量场Z/U/V值求值 ---
        def get_values_from_formula_or_var(cfg, value_key='variable', formula_key='formula'):
            formula = cfg.get(formula_key, '').strip()
            variable = cfg.get(value_key)
        ...
        heatmap_z = get_values_from_formula_or_var(self.heat_cfg)
        contour_z = get_values_from_formula_or_var(self.contour_cfg)
        vector_u = get_values_from_formula_or_var(self.vector_cfg, 'u_variable', 'u_formula')
        vector_v = get_values_from_formula_or_var(self.vector_cfg, 'v_variable', 'v_formula')
...
class PlotWidget(QWidget):
    ...
    def _draw_streamlines(self):
        ...
        # Add colorbar for streamlines if colored by magnitude
        if isinstance(color_data, np.ndarray) and not self.colorbar_obj:
            self.colorbar_obj = self.figure.colorbar(stream_plot.lines, ax=self.ax)
            self.colorbar_obj.set_label(f"流线 ({color_by})")

...
*   **核心性能优化**:
    *   **按需加载**: 重构了核心数据加载逻辑。现在，无论是交互式绘图还是视频导出，程序都只会从数据库中加载当前公式所必需的变量，在派生变量众多时，性能和内存占用得到**数量级**的提升。
    *   **多进程并行**: 派生变量的计算和视频导出等耗时任务采用 **多进程/多线程并行处理** (`ProcessPoolExecutor`/`ThreadPoolExecutor`)，在多核CPU上能显著提升大规模数据集的处理速度。
...
*   **高性能数据访问**:
    *   **按需加载 (On-Demand Loading)**: 作为 v3.5-ProFinal 的核心优化，程序在执行任何操作（绘图、计算）前会先分析公式依赖，确保只从数据库中读取必需的列，极大降低了I/O开销和内存占用。
...
graph TD
    ...
    subgraph Core["核心逻辑 (Core)"]
        D["数据管理器 (DataManager)<br/>负责数据库交互、缓存、变量管理<br/><b>按需加载</b>"]
        E["公式引擎 (FormulaEngine)<br/>负责公式解析与依赖分析"]
    ...
    subgraph Workers["后台工作线程 (Workers - QThread/ProcessPoolExecutor)"]
        P["数据导入/计算线程<br/><b>并行计算</b>"]
        Q["视频导出/批量渲染线程<br/><b>并行渲染</b>"]
    end
...
*   **核心逻辑层 (Core)**: 这是应用的“引擎”。`DataManager` 封装了所有与 SQLite 数据库的交互逻辑，并通过 **按需加载** 和 **智能缓存** 机制保证了数据访问的高性能。`FormulaEngine` 负责解析、验证和评估用户输入的复杂公式。`ComputationCore` 和 `RenderingCore` 则负责将离散的数据点通过插值和计算转换为可以在图上绘制的连续网格化数据。
*   **后台线程 (Workers)**: 对于所有可能耗时的操作，如大规模数据导入、派生变量的全数据集计算、以及视频的逐帧渲染，InterVis 都会创建一个 `QThread` 或使用 `ProcessPoolExecutor` 来执行 **并行计算**。这确保了即使用户在进行数分钟的视频导出，主界面的UI依然能够响应，提供了优秀的用户体验。
...
| **UI位置** | 数据处理 -> 1. 逐帧派生变量 | 数据处理 -> 2. 时间聚合变量 | 数据处理 -> 4. 全局常量 | 数据处理 -> **3. 组合批量计算** |
...
4.  **使用数据处理功能**
    *   切换到 **"数据处理"** 标签页。
    *   在 **"1. 逐帧派生变量"** 框中，输入 `vel_mag = sqrt(u**2 + v**2)`，然后点击“计算并添加”。

...
[![Version](https://img.shields.io/badge/Version-1.8.1--Optimized-blue)](https://github.com/StarsWhere/InterVis)
...
*   ⚙️ **批量处理 (Batch Processing):** 使用 JSON 配置文件自动为不同的可视化设置批量导出视频，并通过**应用内对话框**方便地选择配置。
*   ⚡ **性能优化 (Performance-Focused):**
    *   **核心性能提升:** 自定义全局统计计算采用**单遍算法**，速度提升一个数量级。
    *   **内存优化:** 视频导出采用**临时文件流**，极大降低了内存消耗，支持超长视频导出。
...
公式引擎支持所有基本数学运算、科学常数以及丰富的函数库（三角函数、对数、指数等）。当公式输入有误时，输入框会高亮显示，而不会清空您的输入。
...
在基础统计计算完成后，您可以定义自己的、更复杂的全局常量。**得益于单遍算法优化，即使定义多个复杂的常量，计算过程也极为迅速**。这对于计算那些需要全时域数据聚合的物理量（如雷诺应力）至关重要。
...
*   **视频导出:** 指定开始和结束帧、帧率 (FPS)，以及**渲染网格分辨率**，将动画导出为 MP4 视频。更高的渲染分辨率会生成更精细的视频，但耗时更长。**得益于内存优化，现在可以导出任意长度和分辨率的视频，而不会耗尽内存。**
*   **统计导出:** 将全局统计的计算结果（包括自定义常量的定义和值）导出为易于阅读的 `.csv` 文件。
...
2.  点击“批量视频导出...”，在**应用内对话框**中一次性选择所有这些配置文件。
3.  InterVis 将自动为每个配置文件加载设置并渲染出对应的视频。

![Batch Export Dialog](png/feature_batch_dialog.png)
> *通过应用内对话框选择用于批量导出的配置文件。*
> *Selecting configuration files for batch export via the in-app dialog.*
...
*   **数据IO加速 (Data I/O Acceleration):** PyArrow (推荐)
...
*   [x] ~~**代码重构 (Code Refactoring):**~~ **(v1.7 已实现)**
*   [x] ~~**性能优化与稳定性修复 (Performance Tuning & Stability Fixes):**~~ **(v1.8 已实现)**
